                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="genre" id="genre" required
                            ...
                            @php
                                $genres = ['Action', 'Sci-Fi', 'Drama', 'Comedy', 'Horror', 'Romance', 'Thriller', 'Anime', 'Adventure', 'Fantasy'];
                                $currentGenre = old('genre', $film->genre);
                            @endphp
                            <option value="" disabled {{ empty($currentGenre) ? 'selected' : '' }} class="bg-zinc-900 text-slate-400">Pilih Genre</option>
                            @foreach($genres as $g)
                                <option value="{{ $g }}" {{ $currentGenre === $g ? 'selected' : '' }} class="bg-zinc-900 text-slate-100">{{ $g }}</option>
                            @endforeach
                            @if(!empty($currentGenre) && !in_array($currentGenre, $genres))
                                <option value="{{ $currentGenre }}" selected class="bg-zinc-900 text-slate-100">{{ $currentGenre }}</option>
                            @endif
                        </select>
